...
function ShoppingCart() {
  ...
  useEffect(() => {
    const newTotal = items.reduce(
      (acc, item, index) => acc + item.offer * quantities[index],
      0
    );
    setTotal(newTotal);
  }, [items, quantities]);
  ...
  const plus = (index) => {
    setQuantities((quantities) =>
      quantities.map((qty, i) => (i === index && qty < 5 ? qty + 1 : qty))
    );
  };
  ...
  return (
    <>
      <div className="mainContainer">
        ...
        <p>
          *All prices are inclusive of VAT @ 14% except for products & services
          that are zero rated
        </p>
      </div>
    </>
